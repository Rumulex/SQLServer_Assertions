--Assertions for Azire Data Factory  documentation to perform after repository upgrade and importing changes
...
DECLARE @DatabaseId INT = 41 -- Declare database_id from dbo.databases appropriate to tested documentation
DECLARE @TableId INT = 3186 -- Declare table_id from dbo.databases appropriate to tested documentation (the main table used for most or all manual objects)
...
WHERE [database_id] = @DatabaseId
  AND [procedure_id] IN (388); -- Declare procedure_id(s) from dbo.procedures appropriate to tested documentation
...
WHERE [table_relation_id] = 628 -- Declare appropriate ids from dbo.tables_relations appropriate to tested documentation
    AND [pk_table_id] = 3187
    AND [fk_table_id] = 3186
    AND [source] = 'USER'
    AND [status] = 'A';
...
IF EXISTS (
    ...
    WHERE [unique_constraint_id] = 838 -- Declare appropriate unique_constraint_id from dbo.unique_constraints] appropriate to tested documentation
...
IF EXISTS (
    SELECT 1
    FROM [dataedo_meta_upgrade_cs].[dbo].[unique_constraints]
    WHERE [unique_constraint_id] = 839 -- Declare appropriate unique_constraint_id from dbo.unique_constraints] appropriate to tested documentation
    AND [table_id] = @TableId -- Declare appropriate table_id (if different then declared) from dbo.tables appropriate to tested documentation (object that has keys added)
    AND [source] = 'USER'
    AND [primary_key] = 0
    AND [status] = 'A'
...
-- Columns module
DECLARE @ColumnId INT = 51712; -- Declare appropriate column_id from dbo.columns appropriate to tested documentation
...
DECLARE @LineageProcessId INT = 2607; -- Declare appropriate process_id from dbo.data_processes appropriate to tested documentation (search by plain_description)
...
DECLARE @ObjectFlowId1 INT = 3186; -- Declare appropriate object_id from dbo.data_flows appropriate to tested documentation
DECLARE @ObjectFlowId2 INT = 3187; -- Declare appropriate object_id from dbo.data_flows appropriate to tested documentation
DECLARE @InflowId INT = 3163; -- Declare appropriate inflow_id from dbo.data_flows appropriate to tested documentation
DECLARE @OutflowId INT = 3164; -- Declare appropriate outflow_id from dbo.data_flows appropriate to tested documentation
